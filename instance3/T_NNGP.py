...
def T_Baseline(knowledge_base, weight_u, X_train,y, toolbox,pset,match_path) :
    ...
    for g in range(ngen):
        sample = np.random.choice(index, size=len(index), replace=False)
        ...
        for i in range(len(pop[0])):
            try:
                ...
                pop[0][i].fitness.values = Instance1_PDEs(toolbox, X_train[0], X_train[1], pop[0][i], pop[1][i],match_path)
        ...
        for i in range(len(offspring[0])):
            if not offspring[0][i].fitness.valid or not offspring[0][i].fitness.valid:
                try:
                    ...
                    offspring[0][i].fitness.values = Instance1_PDEs(toolbox,X_train[0], X_train[1], offspring[0][i], offspring[1][i],match_path)
